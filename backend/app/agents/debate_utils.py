...
async def call_debate_llm(
    ...
    try:
        ...
        return _parse_debate_response(response.content)
...
def _parse_debate_response(response_text: str) -> tuple[Optional[str], Optional[float], list[str]]:
    ...
    try:
        ...
        argument_match = re.search(r'"?argument"?\s*:\s*"([^"]+)"', response_text, re.IGNORECASE | re.DOTALL)
        argument = argument_match.group(1) if argument_match else None

        evidence_match = re.search(r'"?evidence_cited"?\s*:\s*\[(.*?)\]', response_text, re.IGNORECASE | re.DOTALL)
        evidence_cited = []
        if evidence_match:
...
def generate_fallback_pro_fraud(evidence: AggregatedEvidence) -> dict:
    ...
    risk_mappings = {
        "critical": (0.90, f"La transacción presenta riesgo CRÍTICO (puntaje {composite_score}/100). "
                    "Múltiples señales de alto riesgo sugieren alta probabilidad de fraude. "
                    "Se recomienda bloqueo inmediato."),
        "high": (0.75, f"La transacción presenta riesgo ALTO (puntaje {composite_score}/100). "
                 "Señales combinadas indican probabilidad considerable de fraude. "
                 "Verificación adicional requerida."),
        "medium": (0.55, f"La transacción presenta riesgo MEDIO (puntaje {composite_score}/100). "
                   "Algunas señales de riesgo presentes. Monitoreo recomendado."),
        "low": (0.30, f"La transacción presenta riesgo BAJO (puntaje {composite_score}/100). "
                "Señales de riesgo mínimas. Probabilidad de fraude reducida."),
    }

    confidence, argument = risk_mappings.get(
        risk_category, (0.50, "Nivel de riesgo no clasificado. Análisis adicional requerido."),
    )
...
def generate_fallback_pro_customer(evidence: AggregatedEvidence) -> dict:
    ...
    risk_mappings = {
        ...
        "low": (0.85, f"El puntaje de riesgo es bajo ({composite_score}/100). "
                "Las señales de fraude son mínimas. Alta probabilidad de transacción legítima."),
    }

    confidence, argument = risk_mappings.get(
        risk_category, (0.50, "Nivel de riesgo no clasificado. Revisión recomendada."),
    )
    ...
    logger.info("pro_customer_fallback_generated", risk_category=risk_category, confidence=confidence)
    return {
