...
Uses llm_port from config["configurable"] and threat providers from config or settings fallback.
"""
...
from app.application.prompts.threat import THREAT_ANALYSIS_PROMPT
...
async def external_threat_agent(state: OrchestratorState, config: RunnableConfig = None) -> dict:
    ...
    try:
        ...
        # Get providers from config or build from settings
        threat_providers = configurable.get("threat_providers")
        if not threat_providers:
            threat_providers = _get_enabled_providers()

        logger.info("providers_initialized", providers=[p.provider_name for p in threat_providers])

        all_sources = await _gather_threat_intel(threat_providers, transaction, transaction_signals)
...
async def _call_llm_for_threat_analysis(
    ...
    if not llm_port:
        logger.warning("external_threat_no_llm_port_in_config")
        return None, "No LLM port configured"

    content, _ = await llm_port.invoke(prompt, agent_name="external_threat")
